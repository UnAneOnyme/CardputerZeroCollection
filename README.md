# Cardputer Zero ~~repository~~ collection
A collection of multiple repositories & links related to Cardputer Zero by M5Stack

---

![Cardputer](https://cdn.shopify.com/s/files/1/0056/7689/2250/files/One-Click_to_Build_Share_and_Run_6d5a481a-6e9c-4230-9284-e1ad000863e3.png)

---

## Links
### *Software-related* :
- [CardputerZero Hub - Cardputer Zero's Online Web UI & App Store](https://cardputerzero.github.io/)

### *Shop/Sales-related* :
- [M5Stack official shop](https://shop.m5stack.com/)
- [Cardputer Zero Super Early Bird Kickstarter reservations](https://m5stack.com/cardputerzero)
- [Check your Cardputer Zero Super Early Bird reservation](https://vm.pledgebox.com/query/Cardputer-Zero)
- [Cardputer Zero Kickstarter page](https://www.kickstarter.com/projects/m5stack/cardputerzero)

### *Communication* :
- [M5Stack's official website](https://m5stack.com/)
- [M5Stack on X](https://x.com/M5Stack) | [M5Stack on X through XCancel](https://xcancel.com/M5Stack)
- [M5Stack on LinkedIn](https://www.linkedin.com/company/m5stack)
- [M5Stack on Instagram](https://www.instagram.com/m5stack)
- [M5Stack on Facebook](https://www.facebook.com/M5Stack) | [Cardputer Zero Launch Group on Facebook](https://www.facebook.com/groups/960129956410838)
- [M5Stack on YouTube](https://www.youtube.com/m5stack)
- [M5Stack on TikTok](https://www.tiktok.com/@m5stack)
- [M5Stack on 小红书/RedNote](https://www.xiaohongshu.com/user/profile/65fc4a98000000000b00e424)
- [M5Stack on Hackster](https://www.hackster.io/m5stack)
- [M5Stack on Discord](https://discord.gg/ysQAWBUE9Q)

### *GitHub* :
- [M5Stack's GitHub organization](https://github.com/m5stack)
- [Cardputer Zero's GitHub organization](https://github.com/CardputerZero)
- [eggfly's GitHub profile](https://github.com/eggfly)
- [dianjixz's GitHub profile](https://github.com/dianjixz)
- [zhuzhe1983's GitHub profile](https://github.com/zhuzhe1983)
- [Cardputer Zero repo collection - This repo as a star list](https://github.com/stars/UnAneOnyme/lists/cardputer-zero)
- [Cardputer Zero repo collection - This repo, featured in Cardputer Zero's organization](https://github.com/CardputerZero/CardputerZeroCollection)

---
## Repositories
### *Platform* :
- [`CardputerZero/M5CardputerZero-Launcher` - Application Launcher & Demo](https://github.com/CardputerZero/M5CardputerZero-Launcher)
- [`CardputerZero/M5CardputerZero-Emulator` - Basic Web & Desktop Cardputer Zero emulator](https://github.com/CardputerZero/M5CardputerZero-Emulator)

### *Build & Dev Tools* :
- [`m5stack/M5Stack_Linux_Libs` - C/C++ M5Stack application dev framework](https://github.com/m5stack/M5Stack_Linux_Libs)
- [`m5stack/CardputerZero-AppBuilder` - Build system & Application dev toolkit](https://github.com/m5stack/CardputerZero-AppBuilder)
- [`eggfly/CardputerZero-AppBuilder-Release` - CI/CD Wrapper for AppBuilder](https://github.com/eggfly/CardputerZero-AppBuilder-Release)
- [`CardputerZero/skill` - Codex skills for Cardputer Zero dev](https://github.com/CardputerZero/skill)

### *System & OS* :
- [`m5stack/m5stack-linux-dtoverlays` - M5Stack devices Device Tree Overlay](https://github.com/m5stack/m5stack-linux-dtoverlays)
- [`CardputerZero/pi-gen` - Tool to generate M5Stack-oriented Raspberry Pi OS images](https://github.com/CardputerZero/pi-gen)
- [`CardputerZero/m5stack-imager` - M5Stack-oriented imaging software for Raspberry Pi](https://github.com/CardputerZero/m5stack-imager)
- [`dianjixz/cm0-firmware` - Pre-compiled binaries of kernel & modules for CM0](https://github.com/dianjixz/cm0-firmware)
- [`CardputerZero/circle-boot-splash` - Bare metal boot splash screen for Cardputer Zero](https://github.com/CardputerZero/circle-boot-splash)
- [`CardputerZero/u-boot` - Open source bootloader for embedded systems](https://github.com/CardputerZero/u-boot)
- [`CardputerZero/cardputer-zero-os` - Cardputer Zero system profile for Debian/Raspberry Pi OS](https://github.com/CardputerZero/cardputer-zero-os)

### *App Distribution/Registry* :
- [`CardputerZero/packages` - APT Repository for Cardputer Zero applications](https://github.com/CardputerZero/packages)
- [`m5stack/CardputerZeroRepository` - Cardputer Zero's `.deb` Debian repository](https://github.com/m5stack/CardputerZeroRepository)

### *Applications* :
- [`CardputerZero/AppStore` - Application to browse & install apps from Cardputer Zero](https://github.com/CardputerZero/AppStore)
- [`eggfly/CardputerZero-NC2000` - 文曲星/Wenquxing NC2000 e-Dict Emulator](https://github.com/eggfly/CardputerZero-NC2000)
- [`CardputerZero/StreamPlayer` - Stream player app](https://github.com/CardputerZero/StreamPlayer)
- [`CardputerZero/AudioSpectrum` - Audio spectrum visualizer](https://github.com/CardputerZero/AudioSpectrum)
- [`CardputerZero/Calculator` - Calculator app](https://github.com/CardputerZero/Calculator)
- [`CardputerZero/2048` - Classic 2048 game](https://github.com/CardputerZero/2048)
- [`CardputerZero/trail-mate` - Outdoor assistant - GPS, LoRa Chat, Maps and more](https://github.com/CardputerZero/trail-mate)
- [`CardputerZero/VibApp` - VibApp Launcher & Client](https://github.com/CardputerZero/VibApp)
- [`CardputerZero/LoFiBox-Zero` - Linux-oriented Cardputer Zero's Music player](https://github.com/CardputerZero/LoFiBox-Zero)
- [`CardputerZero/Calendar` - Calendar application for Cardputer Zero](https://github.com/CardputerZero/Calendar)
- [`geo-tp/Retro-Zero` - Libretro-based emulator frontend for Cardputer Zero](https://github.com/geo-tp/Retro-Zero)
- [`W5DMH/microjs8` - A JS8 transceiver controller for Cardputer Zero](https://github.com/W5DMH/microjs8)
- [`JdaieLin/cardputer-xiaozhi` - XiaoZhi voice assistant ported to M5Cardputer](https://github.com/JdaieLin/cardputer-xiaozhi)
- [`JdaieLin/cardputer-pwnagotchi` - Pwnagotchi client app for Cardputer Zero](https://github.com/JdaieLin/cardputer-pwnagotchi)

### *Examples* :
- [`eggfly/CardputerZero-Examples` - App demos for Cardputer Zero](https://github.com/eggfly/CardputerZero-Examples)

### *Websites & Docs* :
- [`CardputerZero/cardputerzero.github.io` - Cardputer Zero's online app store & Web UI](https://github.com/CardputerZero/cardputerzero.github.io)

### *Launcher Replacements & Full OSes* :
This special category is dedicated to repos containing software that replaces the Launcher as well as full OSes.
- [`7h30th3r0n3/Raspyjack` - Offensive toolkit with Web UI, payload launcher & payload](https://github.com/7h30th3r0n3/Raspyjack)
- [`jayis1/Zero-Day-OS-` - Lightweight pentesting OS for Cardputer Zero](https://github.com/jayis1/Zero-Day-OS-)
- [`CardputerZero/cardputer-zero-shell` - Post-login GUI shell for Cardputer Zero](https://github.com/CardputerZero/cardputer-zero-shell)

---
## About Cardputer Zero
Cardputer Zero & Zero Lite are upcoming credit-card/palm-sized handheld computers made by [M5Stack](https://m5stack.com/) & powered by a [Raspberry Pi Compute Module Zero Lite](https://www.raspberrypi.com/products/compute-module-zero/).

### Specs\* :
| Hardware | Spec | Available on Lite\*\* |
|:---:|:---|:---:|
| SoC | [RP3A0](https://www.raspberrypi.com/documentation/computers/processors.html#rp3a0) ([BCM2837](https://www.raspberrypi.com/documentation/computers/processors.html#bcm2837)) - Same chip as in Pi Zero 2W <br> quad-core Cortex-A53 @ 1 GHz (ARMv8-A - aarch64) | ✅ |
| RAM | 512MB LPDDR2 ([PoP](https://en.wikipedia.org/wiki/Package_on_a_package), integrated in RP3A0) | ✅ |
| Storage | microSD card slot (SDIO)\*\*\* | ✅ |
| microSD card | 32GB microSD card included with non-Lite model\*\*\*\* | ❌ |
| Display | 1.9" 320x170 RGB565 LCD driven by an ST7789v3 | ✅ |
| Keyboard | 46-key matrix keyboard | ✅ |
| Battery | 3.7V 1500mAh Li-Po + BQ27220 fuel gauge <br> [life estimated at around 3h by sn0ren](https://youtu.be/lhS0trmBAAU?t=206) | ✅ |
| Speaker | 1W Speaker | ✅ |
| Microphone | MEMS Microphone | ✅ |
| Camera | Sony IMX219 (8MP 3280x2464) - 4 lanes CSI interface | ❌ |
| Wireless | 2.4GHz Wi-Fi 802.11 b/g/n, Bluetooth 5.0 & BLE | ✅ |
| IMU | BMI270 (gyroscope + accelerometer) | ❌ |
| RTC | RX8130CE | ✅ |
| Infrared | Infrared TX & RX (Transmit & Receive) | ✅ |
| Codecs | Video : Decode H.264/MPEG-4 @ 1080P 30 FPS <br> Video : Encode H.264 @ 1080P 30 FPS <br> Audio : ES8389 | ✅ |
| External Ports & Extensions | - 1x USB-A (USB 2.0, Host/Slave switchable) <br> - 2x USB Type-C (USB 2.0, Host/Slave switchable) <br> - 1x 10/100M Ethernet RJ45 <br> - 1x HDMI (Full-size) <br> - 1x Jack TRS 3.5mm audio output <br> - 1x HY2.0-4P GROVE Port (I²C/UART switchable) <br> - 1x 2.54-14P Bus (SPI, UART, I²C, USB, GPIO & 5V)| ✅ |

\* As I'm writing this during device development, these specs are subject to change.

\*\* Whether or not the feature is available on Cardputer Zero Lite. If not, said feature is limited to Cardputer Zero.

\*\*\* While I could not find any max size in device's documentation (both from Raspberry Pi & M5Stack). As stated in [RPi's getting started guide](https://www.raspberrypi.com/documentation/computers/getting-started.html), 2TB should be a hard limit as more would exceed the limits of MBR.

\*\*\*\* 32GB is not a limit, it's just the size of the cards they decided to package with non-Lite models. As of 2026-05-14, the brand is unknown.

---
## About this repo
If:
- You want to add something to the list
- Some information is wrong and needs to be removed/updated
- You want something removed (e.g., your work or your account)
- You got any questions

Feel free to edit & open a pull request, open a GitHub issue or contact me on Discord (@`1_ane_onyme`)

**Last update on 2026-05-25**
